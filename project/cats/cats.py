...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    selected = [p for p in paragraphs if select(p) == True]
    if k < len(selected):
        return selected[k]
    return ''
...
def about(keywords):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def if_contain_keywords(paragraphs):
        list_paragraphs = lower(remove_punctuation(paragraphs)).split()
        for i in keywords:
            if i in list_paragraphs:
                return True
        return False
    return if_contain_keywords
...
def accuracy(typed, source):
    ...
    if length_typed_words == 0:
        if length_source_words == 0:
            return 100.0
        else:
            return 0.0
    for i in range (length_typed_words):
        if i >= length_source_words:
            break
        elif typed_words[i] == source_words[i]:
            accuracy_times += 1
    return accuracy_times / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    minimum_diff_word = min(word_list, key = lambda word :diff_function(typed_word, word, limit))
    if diff_function(typed_word, minimum_diff_word, limit) <= limit:
...
def furry_fixes(typed, source, limit):
    ...
    if length_typed > length_source:
        return furry_fixes(typed[:length_source], source, limit - (length_typed - length_source)) + (length_typed - length_source)
    elif length_typed < length_source:
        return furry_fixes(typed, source[:length_typed], limit - (length_source - length_typed)) + (length_source - length_typed)
    elif limit < 0:
        return 0
    elif length_source == length_typed == 0:
        return 0
    else:
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if limit < 0 or len(typed) == len(source) == 0: # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 0
        # END
...
def report_progress(typed, source, user_id, upload):
    ...
    while k < min(length_typed, length_source):
        if typed[k] == source[k]:
            correct_times += 1
        else:
            break
        k += 1
    progress = correct_times / length_source
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM
    times = []
    for k in range(len(timestamps_per_player)):
        time = [[timestamps_per_player[k][i] - timestamps_per_player[k][i - 1] for i in range(1, len(timestamps_per_player[k]))]]
        times  += time
        # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        player_num = min(player_indices, key = lambda x: get_time(times, x, word_index))
        result[player_num].append(words[word_index])
    return result
    # END PROBLEM 10
